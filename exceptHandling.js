...
while (true) {
    ...
    if (action === "add") {
        ...
        if(animal === "" || !isNaN(animal)) {
        throw new Error("The animal must have a name!")
        }
        let fee = Number(readlineSync.question("Enter the adoption fee: "));
        if(fee < 0 || isNaN(fee)) {
            throw new Error("Please enter valid dollar amount.")
        }                 
...
Invalid Input Errors:
  ...
  // It gives the throw error of "Invalid animal name or adoption fee!" and ends the ends the program

  What happens if the user tries to find the fee for an animal that hasn’t been added?

  // It gives the throw error "Animal not found in records!" and halts the program

Code Flow Problems:
  What happens if the program throws an exception? Does the rest of the code continue running?

  //    The programs comes to halt
